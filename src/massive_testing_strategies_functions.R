...
# Group results
estimate_pooling_tests <- function(group_size, p, pop, se, s_loss, sp) {
    ...
    for (sens in sens_model) {
      for (spec in spec_model) {
        ...
        for (prob_symp_more_5d in c(0.25, 0.5, 0.75)) {
          ...
          prob_res_neg_given_high_risk_ag <- (1 - spec) * prob_res_neg_ag /
            prob_model_high_risk

          prob_res_pos_given_low_risk_ag <- (1 - sens) * prob_res_pos_ag /
            prob_model_low_risk

          prob_res_pos_given_high_risk_lamp <- sens * prob_res_pos_lamp /
            prob_model_high_risk

          prob_res_neg_given_high_risk_lamp <- (1 - spec) * prob_res_neg_lamp /
            prob_model_high_risk

          prob_res_pos_given_low_risk_lamp <- (1 - sens) * prob_res_pos_lamp /
            prob_model_low_risk

          prob_res_pos_given_high_risk_pcr <- sens * prob_res_pos_pcr /
            prob_model_high_risk
          ...
          if (strategy == 1) {
            ...
            number_pcr_tests <- people_high_risk * (prob_symp_more_5d +
            ...
            number_positive_reported <- people_high_risk * prob_symp_less_5d * (
          ...
          # Strategy 2 --------------------------------------------------------
            ...
            pooling_tests_low_risk <- do.call("rbind", pooling_tests_low_risk)
        ...
        # Strategy 4 ----------------------------------------------------------
        if (strategy == 4) {
          number_alt_tests <- people_high_risk


          number_pcr_tests <- people_high_risk * prob_res_neg_given_high_risk_lamp
          ...
          total_cost_pcr <- cost_pcr * number_pcr_tests
          total_cost_alt <- cost_lamp * number_alt_tests
          total_cost <- total_cost_alt + total_cost_pcr
        ...
        # Strategy 6 ------------------------------------------------------
          ...
          number_alt_tests <- people +
            people_low_risk * prob_res_pos_given_low_risk_lamp
          ...
          number_positive_reported <- people_high_risk *
            ...
            people_low_risk *
              (prob_res_pos_given_low_risk_lamp * prob_res_pos_lamp)
...
# ppv_npv_fdr <- function(prevalence, sens, specificity) {
#   ppv <- sens * prevalence /
#     (sens * prevalence + (1 - specificity) * (1 - prevalence))
...
#
# evaluacion_costo <-
#   function(prevalence_vec = seq(0, 0.5, by = 0.01),
#            sensitivity_vec = c(0.25, 0.5, 0.75, 0.95),
#            specificity_vec = c(0.25, 0.5, 0.75, 0.95),
#            people = 1000,
#            cost_pcr = 100,
#            cost_antigen = 50,
#            cost_lamp = 20,
#            alternative_test = "LAMP",
#            negative.strategy  = c("nothing", "retest", "pooling"),
#            system = "public",
#            pooling = FALSE,
#            npools = 5,
#            limit_costs = TRUE) {
...
#     )
#
#     for (sensitivity in sensitivity_vec) {
#       for (specificity in specificity_vec) {
#         # Current public strategy with preselection + antigen
#         if (alternative_test == "antigen" &
#             system == "public" &
#             (
#               negative.strategy == "nothing" |
#               negative.strategy == "retest" |
#               negative.strategy == "pcr"
#             )) {
#           # Probability before model,  there are 3 options
#           # pos = antigen,  neg = pcr,  pos = antigen + pcr
#           prob_before_model <- lapply(
#             X = prevalence_vec,
#             FUN = function(p) {
#               pos <- sensitivity * p + (1 - specificity) * (1 - p)
#               neg <- (1 - sensitivity) * p + specificity * (1 - p)
#               c(pos, neg, pos)
#             }
#           )
#
#           # Probability after model. Depends on the prevalence
#           prob_after_model <- lapply(X = prevalence_vec,
#                                      FUN = ppv_npv_fdr,
#                                      sensitivity,
#                                      specificity)
#
#           cost_total <- mapply(function(x, y, z) {
#             people * sum(x * y *
#                            c(
#                              cost_antigen,
#                              if (negative.strategy == "nothing") {
#                                0
#                              } else if (negative.strategy == "retest") {
#                                cost_antigen
#                              } else if (negative.strategy == "pcr") {
#                                cost_pcr
#                              } else{
#                                stop("Error")
#                              },
#                              cost_antigen + cost_pcr
#                            ))
#           },
#           prob_before_model,
#           prob_after_model)
#
#
#           prob_cost <- mapply(function(x, y) {
#             d <- data.frame(x * y * c(cost_antigen,
#                                       cost_pcr,
#                                       cost_antigen + cost_pcr))
#           },
#           prob_before_model,
#           prob_after_model)
#
#           prob_cost <- as.data.frame(do.call(rbind, prob_cost))
#           colnames(prob_cost) <-
#             c("cost_antigen", "cost_pcr", "cost_antigen_pcr")
#           rownames(prob_cost) <- NULL
#
#
#           prob_after_model <- do.call(rbind, prob_after_model)
#           colnames(prob_after_model) <- c("ppv", "npv", "fdr")
#
#           prob_before_model <-
#             do.call(rbind, prob_before_model)[, 1:2]
#           colnames(prob_before_model) <- c("pos", "neg")
#
#           df_out <- rbind(df_out,
#                           cbind(
#                             data.frame(
...
#                           ))
#
#         } else   if (alternative_test == "antigen" &
#                      system == "public" &
#                      negative.strategy == c("pooling")) {
#           # Probability before model,  there are 3 options
#           # pos = antigen,  neg = pcr,  pos = antigen + pcr
#           prob_before_model <- lapply(
#             X = prevalence_vec,
...
#           # Probability after model. Depends on the prevalence
#           prob_after_model <-
#             lapply(prevalence_vec, function(prevalence) {
...
#                 (1 - sensitivity) * prevalence + specificity * (1 - prevalence)
#               test <-
#                 prev.results(npools, neg, prevalencia = 1 - npv)
#               if (limit_costs) {
#                 c(ppv,
#                   ifelse(test$total.tests > neg, neg, test$total.tests),
#                   fdr)
#               }
#               else{
#                 c(ppv, test$total.tests, fdr)
#               }
#
#             })
#
...
#
#           cost_total <- mapply(function(x, y, z) {
#             people * sum(x * c(0, cost_antigen))
#           }, prob_before_model)
#
#
#           prob_cost <- mapply(function(x, y) {
#             d <- data.frame(x * c(0, cost_antigen))
#           }, prob_before_model)
#
#
#           prob_cost <- as.data.frame(do.call(rbind, prob_cost))
#           colnames(prob_cost) <-
#             c("cost_positive_predicton", "cost_antigen")
#           rownames(prob_cost) <- NULL
#
#           prob_before_model <-
#             do.call(rbind, prob_before_model)[, 1:2]
...
#
#
#         } else if (alternative_test == "LAMP" & pooling == TRUE) {
#           prob_before_model <-
#             lapply(prevalence_vec,
#                    function(p) {
#                      pos <- sensitivity * p + (1 - specificity) * (1 - p)
...
#           prob_cost <- mapply(function(x, y) {
#             d <-
#               data.frame(x * y * c(cost_pcr, cost_lamp))
#           },
#           prob_before_model,
#           prob_pooling)
#
...
#
#
# # sistema_publico_preseleccion <-
# #   function(prevalence_vec = seq(0, 1, by = 0.01),
# #            sensitivity_vec = c(0.25, 0.5, 0.75, 0.95),
# #            specificity_vec = c(0.25, 0.5, 0.75, 0.95),
# #            people = 1000) {
# #     cost_pcr <- 100
...
# #       for (specificity in specificity_vec) {
# #         prob_after_model <-
# #           lapply(prevalence_vec, ppv_npv_fdr, sensitivity, specificity)
# #         prob_before_model <- lapply(prevalence_vec,
# #                                     function(p) {
# #                                       # r<- runif(1)
# #                                       # c(r, 1-r, r)
# #                                       pos <-
...
# #
# #
# #         cost <- mapply(function(x, y, z) {
# #           people * sum(x * y *
# #                          c(cost_antigen, cost_pcr, cost_antigen + cost_pcr))
# #         },
# #         prob_before_model,
# #         prob_after_model)
# #
...
# #         prob_cost <- mapply(function(x, y) {
# #           d <-
# #             data.frame(x * y * c(cost_antigen, cost_pcr, cost_antigen + cost_pcr))
# #         },
# #         prob_before_model,
# #         prob_after_model)
# #
...
#
#
# # Consultar si en el modelaje de esste se incluye el sistema de salud publico, o se asume ese costo como cero
# # sistema_privado_preseleccion_antigeno <-
# #   function(prevalence_vec = seq(0, 1, by = 0.01),
# #            sensitivity_vec = c(0.25, 0.5, 0.75, 0.95),
# #            specificity_vec = c(0.25, 0.5, 0.75, 0.95),
# #            people = 1000) {
...
# prev.results <-
#   function(pool, pop, other.data = pool.sen, prevalencia) {
#     sensitivity <- other.data[which(pool.sen[, 1] == pool), 2]
#     prev <- as.data.frame(prevalencia)
#     colnames(prev) <- "prevalence"
#
#     out <- lapply(prevalencia,
#                   function(X) {
#                     group.results(pool,
#                                   m = X,
#                                   pop = pop,
...
#       mutate(pos.found = pop * prevalence - 0) # numb.undetected.pos
#
#     return(out)
#   }
#
# pool.sen <- as.data.frame(rbind(c(1, 1),
#                                 c(5, 1),
#                                 c(10, 1),
#                                 c(20, 1)))
#
...
#
#
# # sistema_publico_preseleccion_lamp_pooling <-
# #   function(prevalence_vec = seq(0, 1, by = 0.01),
# #            sensitivity_vec = c(0.25, 0.5, 0.75, 0.95),
# #            specificity_vec = c(0.25, 0.5, 0.75, 0.95),
# #            people = 1000,
...
# #             neg <-
# #               (1 - sensitivity) * prevalence + specificity * (1 - prevalence)
# #             test <- prev.results(5, neg, prevalencia = 1 - npv)
# #             c(1,
# #               ifelse(test$total.tests > neg, neg, test$total.tests))
# #           })
# #
...
# #         prob_cost <- mapply(function(x, y) {
# #           d <-
# #             data.frame(x * y * c(cost_lamp, cost_lamp))
# #         },
# #         prob_before_model,
# #         prob_pooling)
# #
